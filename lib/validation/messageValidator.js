...
const validateMessage = async (message, interestedSaga, requiredContextProps = [], requiredDecorationsProps = [], rollbackHandler) => {
    ...
    try{
        if (message.saga !== interestedSaga) {
            logger.warn('Not interested in this saga', { correlationId: message.correlationId, saga: message.saga });
        ...
        if (message.lastServiceDecoration === currentServiceName) {
            logger.warn('Message last decorated by this service', { correlationId: message.correlationId });
        ...
        if (errorDecoration) {
            ...
            if (alreadyProcessed) {
                logger.warn('Message already processed, triggering rollback', { correlationId: message.correlationId });
        ...
        if (message.decorations.some(decoration => decoration.service === currentServiceName)) {
            logger.warn('Message already decorated by this service', { correlationId: message.correlationId });
            return { isValid: false, reason: 'Already decorated' };
        }

        for (const prop of requiredContextProps) {
            if (message.context[prop] === undefined) {
                logger.warn('Missing required context property', { correlationId: message.correlationId, prop });
        ...
        for (const prop of requiredDecorationsProps) {
            ...
            if (!decorationExists) {
                logger.warn('Missing required decoration property', { correlationId: message.correlationId, prop });
