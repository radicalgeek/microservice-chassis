...
async function connect() {
  const segment = process.env.NODE_ENV === 'development'
    ? new MockSegment('RabbitMQ-Connect')
...
const retryConnect = async (maxRetries = 5, delay = 1000) => {
  const segment = process.env.NODE_ENV === 'development'
    ? new MockSegment('RabbitMQ-RetryConnect')
...
async function createAndBindQueue(channel) {
  const segment = process.env.NODE_ENV === 'development'
    ? new MockSegment('RabbitMQ-CreateAndBindQueue')
...
async function listenToQueue(channel, callback) {
  const segment = process.env.NODE_ENV === 'development'
  ? new MockSegment('RabbitMQ-ListenToQueue')
...
async function publishMessage(channel, message) {
  const segment = process.env.NODE_ENV === 'development'
  ? new MockSegment('RabbitMQ-PublishMessage')
