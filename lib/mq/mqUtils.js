...
async function connect() {
  const segment = process.env.NODE_ENV === 'development'
    ? AWSXRay.getSegment() || new MockSegment('RabbitMQ-Connect')
    : AWSXRay.getSegment() || new AWSXRay.Segment('RabbitMQ-Connect');
...
const retryConnect = async (maxRetries = 5, delay = 1000) => {
  const segment = process.env.NODE_ENV === 'development'
    ? AWSXRay.getSegment() || new MockSegment('RabbitMQ-RetryConnect')
    : AWSXRay.getSegment() || new AWSXRay.Segment('RabbitMQ-RetryConnect');
...
async function createAndBindQueue(channel) {
  const segment = process.env.NODE_ENV === 'development'
    ? AWSXRay.getSegment() || new MockSegment('RabbitMQ-CreateAndBindQueue')
    : AWSXRay.getSegment() || new AWSXRay.Segment('RabbitMQ-CreateAndBindQueue');
...
async function listenToQueue(channel, callback) {
  const segment = process.env.NODE_ENV === 'development'
  ? AWSXRay.getSegment() || new MockSegment('RabbitMQ-ListenToQueue')
  : AWSXRay.getSegment() || new AWSXRay.Segment('RabbitMQ-ListenToQueue');
...
async function publishMessage(channel, message) {
  const segment = process.env.NODE_ENV === 'development'
  ? AWSXRay.getSegment() || new MockSegment('RabbitMQ-PublishMessage')
  : AWSXRay.getSegment() || new AWSXRay.Segment('RabbitMQ-PublishMessage');
