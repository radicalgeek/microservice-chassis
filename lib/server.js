const apm = require('elastic-apm-node').start({
    ...
    serverUrl: process.env.APM_SERVER, //'http://apm-apm-http.gitlab-managed-apps.svc.cluster.local:8200',
...
function setupRoutes(routeConfig, db, rabbitMqConnection) {
...
async function publishStartupMessages(startupMessages, publishChannel) {
    ...
    for (const messageConfig of startupMessages) {
        if (messageConfig.message && messageConfig.interval) {
            publishMessage(publishChannel, messageConfig.message);
...
async function startService(serviceConfig, db) {
    try{
        ...
        setupRoutes(serviceConfig.routes, db, rabbitMqConnection);
        if (serviceConfig.useMinio && serviceConfig.bucketName) {
            await setupMinio(serviceConfig.bucketName);
        }
        publishStartupMessages(serviceConfig.startupMessages, publishChannel);
